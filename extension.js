 ...
 *
 * Authors:
 * Patrick Libert <patrick.libert@gmail.com>
 */
"use strict";
/** @external imports*/
...
const MyIndicator = Me.imports.MailExtension.MailExtension;
...
var Extension = class {
    constructor() {
        ...
        this._getEmailAccounts(emailAccounts => {
            ...
            this.startTimeout();
            this.initialCheckMail = GLib.timeout_add_seconds(0, 1, () => {
                this._checkMail();
                return false;
            });
        });
    ...
    _checkMail() {
        ...
        for (let account of this.goaAccounts) {
            account.scanInbox();
            this.initialCheckMail = GLib.timeout_add_seconds(0, 1, () => {
                indicator.setMails(account._MailCounts);
               // log(account._MailCounts);
...
function enable() {
    try {
        ...
        Main.panel.addToStatusArea("Gnome_Accounts_with_tray-icon", indicator,0);
    } catch (err) {
        console.error(err);
    }
...
function disable() {
    ...
    try {
        extension.destroy();
        extension = null;
        Main.panel.statusArea["Gnome_Accounts_with_tray-icon"].destroy();
    } catch (err) {
        console.error(err);
    }
